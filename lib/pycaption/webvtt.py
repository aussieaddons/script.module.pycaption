...
class WebVTTReader(BaseReader):
        ...
        for i, line in enumerate(lines):
            ...
            if '-->' in line:
                ...
                try:
                    ...
                    six.reraise(type(e), type(e)(new_message),
                                sys.exc_info()[2])
    ...
    def _parse_timestamp(self, timestamp):
        ...
        if m[2]:
            # Timestamp takes the form of [hours]:[minutes]:[seconds].[
            # milliseconds]
...
class WebVTTWriter(BaseWriter):
    ...
    def write(self, caption_set):
        ...
        return output + '\n'.join(
            [self._write_caption(caption_set, caption) for caption in
             captions])
    ...
    def _layout_groups(self, nodes, caption_set):
        ...
        for i, node in enumerate(nodes):
            if node.type_ == CaptionNode.TEXT:
                ...
                resulting_style = self._calculate_resulting_style(node.content,
                                                                  caption_set)
